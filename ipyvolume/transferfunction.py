...
class TransferFunction(widgets.DOMWidget):
    ...
    rgba = Array(default_value=None, allow_none=True).tag(
        sync=True, **serialize.ndarray_serialization
    )
...
class TransferFunctionJsBumps(TransferFunction):
    ...
    _model_module = Unicode("ipyvolume").tag(sync=True)
    levels = traitlets.List(traitlets.CFloat(), default_value=[0.1, 0.5, 0.8]).tag(
        sync=True
    )
    opacities = traitlets.List(traitlets.CFloat(), default_value=[0.01, 0.05, 0.1]).tag(
        sync=True
    )
    widths = traitlets.List(traitlets.CFloat(), default_value=[0.1, 0.1, 0.1]).tag(
        sync=True
    )
...
class TransferFunctionWidget3(TransferFunction):
    ...
    def recompute_rgba(self, *_ignore):
        ...
        colors = [
            np.array(matplotlib.colors.colorConverter.to_rgb(name))
